        ...
        stage('Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-credentials', passwordVariable: 'gitid', usernameVariable: 'GITHUB_USER')]) {
                    sh 'rm -rf TFG'
                    sh 'git clone --branch produccion https://github.com/JG-Alba/TFG.git'
                    sh '''git config --global --add safe.directory /var/jenkins_home/workspace/prueba/TFG
'''
                    sh 'cp -r Dockerfile wordpress TFG && cd TFG && git add . && git push https://${GITHUB_USER}:${gitid}@github.com/JG-Alba/TFG.git'
                }
